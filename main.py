...
def stage_assets(proj_dir):
    ...
    print("\n  Generating scene images (local SDXL)...")
    img_dir = _assets_path(proj_dir) / "images"
    img_dir.mkdir(parents=True, exist_ok=True)

    # Default model path — user can override via env var
    ...
    if not model_path.exists():
        ...
        print("    Skipping image generation — will use existing images.\n")
    else:
        from tools.base_tool import ToolStatus
        from tools.graphics.local_diffusion import LocalDiffusion
        img_tool = LocalDiffusion()
        if img_tool.get_status() != ToolStatus.AVAILABLE:
            print("    ⚠ LocalDiffusion tool unavailable (diffusers not installed?)")
        else:
            scenes = scene_plan.get("scenes", [])
            generated_count = 0
            for sc in scenes:
                sc_id = sc.get("id", f"scene-{generated_count+1}")
                out_path = img_dir / f"{sc_id}.jpg"
                if out_path.exists():
                    print(f"    ∎ {sc_id}: already exists, skipping")
                    continue

                # Build prompt from scene description + type
                ...
                if result.success:
                    generated_count += 1
                    print(f"    ✅ {sc_id}: image saved")
                else:
                    print(f"    ⚠ {sc_id}: generation failed — {result.error}")
